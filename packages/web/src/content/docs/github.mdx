...
Set the `comment_key` input to make OpenCode maintain a single comment per issue or PR instead. On each run, OpenCode will look for an existing comment it posted with the same key and update it in place. If no matching comment is found, it creates a new one.
