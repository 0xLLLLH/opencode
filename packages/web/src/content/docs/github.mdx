...
- `comment_key`: Optional string used to update an existing comment instead of creating a new one on each run. See [Sticky comments](#sticky-comments) for details.
...
The `comment_key: review-summary` setting ensures that each new push to the PR updates the same review comment rather than creating a new one. See [Sticky comments](#sticky-comments) for more details.
...
By default, OpenCode creates a new comment every time it runs. When you use automated workflows (such as reviewing every PR push), this can produce many similar comments on the same PR.

Set the `comment_key` input to identify a specific comment. On each run, OpenCode will look for an existing comment it posted with the same key and update it in place. If no matching comment is found, it creates a new one.
...
With this configuration, each new push to the PR updates the same review comment rather than adding a new one.

### Choosing a key

The key can be any non-empty string. Use a fixed value when you want one persistent comment per PR, or compose it from GitHub context values to create separate comments for different workflows or jobs.

| Use case | Recommended key |
| --- | --- |
| Single review workflow | `review-summary` |
| Multiple workflows on the same PR | `${{ github.workflow }}` |
| Multiple jobs within one workflow | `${{ github.workflow }}-${{ github.job }}` |

### How it works

OpenCode embeds a hidden HTML anchor in the comment body that encodes a hash of the key:

```html
<!-- opencode:comment-key:sha256:<digest> -->
```

On subsequent runs, OpenCode searches the issue or PR for a bot comment containing the matching anchor, then updates that comment. Because the key is hashed, arbitrary strings including spaces and special characters are safe to use.

Changing the `comment_key` value starts a new comment track — the previous comment is left intact and a new one is created.
